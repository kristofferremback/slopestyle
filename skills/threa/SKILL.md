---
name: threa
description: Use when a task needs Threa workspace memory, messages, streams, attachments, conversations, delegations, channel replies, or the threa CLI and MCP tools.
---
...
Reads are free. Writing into a stream, claiming or settling a delegation, changing labels, or requesting access changes shared state. Do that only when the current request or accepted delegation explicitly authorizes it. A user key speaks as the person, so never post with one without Kris's permission. A bot may publish the expected output of its authorized task with proper agent attribution. When authorization is unclear, inspect and prepare the exact action, then ask in ordinary text.
