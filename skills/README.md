...
The root `hosts.json` restricts managed skills to named machines. Datadog is limited to `kristoffer-mbp-galdera` because it uses that laptop's Galdera credentials. Its `legacy-codex.json` records the exact original local files that the installer may back up during migration. It contains hashes only.
